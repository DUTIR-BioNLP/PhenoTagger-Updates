...
class TransE:
    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            #保存临时结果
            if epoch % 20 == 0:
                with codecs.open("./transE_emb_2024/entity_temp", "w") as f_e:
                    for e in self.entity.keys():
                        f_e.write(e + "\t")
                        f_e.write(str(list(self.entity[e])))
                        f_e.write("\n")
                with codecs.open("./transE_emb_2024/relation_temp", "w") as f_r:
        ...
        print("写入文件...")
        with codecs.open("./transE_emb_2024/entity_768dim_batch400", "w") as f1:
            for e in self.entity.keys():
                f1.write(e + "\t")
                f1.write(str(list(self.entity[e])))
                f1.write("\n")

        with codecs.open("./transE_emb_2024/relation768dim_batch400", "w") as f2:
            for r in self.relation.keys():
                f2.write(r + "\t")
...
if __name__=='__main__':
    ...
    transE = TransE(entity_set, relation_set, triple_list,embedding_dim=512, learning_rate=0.01, margin=1,L1=True)
    transE.emb_initialize()
    transE.train(epochs=500)